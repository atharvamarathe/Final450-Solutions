...
int search(int inorder[],int a,int size) {
...
Node* constructTreeUtil(int pre[],int i,int j) {
    if(i <= j) {
    ...
    root -> left = constructTreeUtil(pre,i+1,mid-1);
    root -> right = constructTreeUtil(pre,mid,j);
...
Node* constructTree(int pre[], int size)
{
    ...
    for(int i = 0;i<size;i++)
        inorder[i] = pre[i];
    Node* root;
    root = constructTreeUtil(pre,0,size-1);
